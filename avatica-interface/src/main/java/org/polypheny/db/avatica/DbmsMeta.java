...
public class DbmsMeta implements ProtobufMeta {
    ...
    private MetaResultSet createMetaResultSet(final ConnectionHandle ch, final StatementHandle statementHandle, Map<String, Object> internalParameters, List<ColumnMetaData> columns, CursorFactory cursorFactory, final Frame firstFrame) {
        final PolyphenyDbSignature<Object> signature = new PolyphenyDbSignature<Object>("", ImmutableList.of(), internalParameters, null, columns, cursorFactory, null, ImmutableList.of(), -1, null, StatementType.SELECT, new ExecutionTimeMonitor(), NamespaceType.RELATIONAL) {
                    /*@Override
    ...
    public MetaResultSet getTables(final ConnectionHandle ch, final String database, final Pat schemaPattern, final Pat tablePattern, final List<String> typeList) {
        ...
        synchronized (connection) {
            ...
            final List<CatalogTable> tables = catalog.getTables(database == null ? null : new Pattern(database), (schemaPattern == null || schemaPattern.s == null) ? null : new Pattern(schemaPattern.s), (tablePattern == null || tablePattern.s == null) ? null : new Pattern(tablePattern.s));
            StatementHandle statementHandle = createStatement(ch);
            return createMetaResultSet(ch, statementHandle, toEnumerable(tables), PrimitiveCatalogTable.class,
    ...
    public MetaResultSet getColumns(final ConnectionHandle ch, final String database, final Pat schemaPattern, final Pat tablePattern, final Pat columnPattern) {
        ...
        synchronized (connection) {
            ...
            final List<CatalogColumn> columns = catalog.getColumns(database == null ? null : new Pattern(database), (schemaPattern == null || schemaPattern.s == null) ? null : new Pattern(schemaPattern.s), (tablePattern == null || tablePattern.s == null) ? null : new Pattern(tablePattern.s), (columnPattern == null || columnPattern.s == null) ? null : new Pattern(columnPattern.s));
            StatementHandle statementHandle = createStatement(ch);
            return createMetaResultSet(ch, statementHandle, toEnumerable(columns), PrimitiveCatalogColumn.class,
                    // According to JDBC standard:
    ...
    public MetaResultSet getSchemas(final ConnectionHandle ch, final String database, final Pat schemaPattern) {
        ...
        synchronized (connection) {
            ...
            final List<CatalogSchema> schemas = catalog.getSchemas(database == null ? null : new Pattern(database), (schemaPattern == null || schemaPattern.s == null) ? null : new Pattern(schemaPattern.s));
            StatementHandle statementHandle = createStatement(ch);
            return createMetaResultSet(ch, statementHandle, toEnumerable(schemas), PrimitiveCatalogSchema.class,
                    // According to JDBC standard:
    ...
    public MetaResultSet getCatalogs(final ConnectionHandle ch) {
        ...
        synchronized (connection) {
            ...
            StatementHandle statementHandle = createStatement(ch);
            return createMetaResultSet(ch, statementHandle, toEnumerable(databases), PrimitiveCatalogDatabase.class,
                    // According to JDBC standard:
                    "TABLE_CAT",
                    // Polypheny-DB specific extensions:
    ...
    public MetaResultSet getTableTypes(final ConnectionHandle ch) {
        ...
        synchronized (connection) {
            ...
            StatementHandle statementHandle = createStatement(ch);
            return createMetaResultSet(ch, statementHandle, enumerable, PrimitiveTableType.class, "TABLE_TYPE");
        }
    }
    ...
    public MetaResultSet getPrimaryKeys(final ConnectionHandle ch, final String database, final String schema, final String table) {
        ...
        synchronized (connection) {
            ...
            StatementHandle statementHandle = createStatement(ch);
            return createMetaResultSet(ch, statementHandle, toEnumerable(primaryKeyColumns), PrimitiveCatalogPrimaryKeyColumn.class,
                    // According to JDBC standard:
    ...
    public MetaResultSet getImportedKeys(final ConnectionHandle ch, final String database, final String schema, final String table) {
        ...
        synchronized (connection) {
            ...
            StatementHandle statementHandle = createStatement(ch);
            return createMetaResultSet(ch, statementHandle, toEnumerable(foreignKeyColumns), PrimitiveCatalogForeignKeyColumn.class,
                    // According to JDBC standard:
    ...
    public MetaResultSet getTypeInfo(final ConnectionHandle ch) {
        ...
        synchronized (connection) {
            ...
            for (PolyType polyType : PolyType.values()) {
                objects.add(new Serializable[]{polyType.getName(), polyType.getJdbcOrdinal(), typeSystem.getMaxPrecision(polyType), typeSystem.getLiteral(polyType, true), typeSystem.getLiteral(polyType, false), null, (short) DatabaseMetaData.typeNullable, // All types are nullable
                        typeSystem.isCaseSensitive(polyType), (short) DatabaseMetaData.typeSearchable, // Making all type searchable; we may want to be specific and declare under PolyType
                        false, false, typeSystem.isAutoincrement(polyType), polyType.getName(), (short) polyType.getMinScale(), (short) typeSystem.getMaxScale(polyType), null, null, typeSystem.getNumTypeRadix(polyType) == 0 ? null : typeSystem.getNumTypeRadix(polyType)});
            }
            return createMetaResultSet(ch, statementHandle, Linq4j.asEnumerable(objects), MetaTypeInfo.class, "TYPE_NAME",   // The name of the data type.
                    "DATA_TYPE",           // The SQL data type from java.sql.Types.
    ...
    public MetaResultSet getIndexInfo(final ConnectionHandle ch, final String database, final String schema, final String table, final boolean unique, final boolean approximate) {
        ...
        synchronized (connection) {
            ...
            StatementHandle statementHandle = createStatement(ch);
            return createMetaResultSet(ch, statementHandle, toEnumerable(catalogIndexColumns), PrimitiveCatalogIndexColumn.class,
                    // According to JDBC standard:
                    "TABLE_CAT",    // The name of the database in which the specified table resides.
    ...
    public StatementHandle prepare(final ConnectionHandle ch, final String sql, final long maxRowCount) {
        ...
        synchronized (connection) {
            ...
            PolyphenyDbSignature signature = new PolyphenyDbSignature<>(sql, avaticaParameters, ImmutableMap.of(), parameterRowType, null, null, null, ImmutableList.of(), -1, null, StatementType.SELECT, null, NamespaceType.RELATIONAL);
    ...
    public ExecuteResult prepareAndExecute(final StatementHandle h, final String sql, final long maxRowCount, final int maxRowsInFirstFrame, final PrepareCallback callback) throws NoSuchStatementException {
        System.out.println("** Verify user's acces **");
    ...
    private void prepare(StatementHandle h, String sql) throws NoSuchStatementException {
        ...
        if (parsed.isA(Kind.DDL)) {
            ...
            Pair<Node, AlgDataType> validated = sqlProcessor.validate(statementHandle.getStatement().getTransaction(), parsed, RuntimeConfig.ADD_DEFAULT_VALUES_IN_INSERTS.getBoolean());
    ...
    private List<MetaResultSet> execute(StatementHandle h, PolyphenyDbConnectionHandle connection, PolyphenyDbStatementHandle statementHandle, int maxRowsInFirstFrame) {
        ...
        if (statementHandle.getSignature().statementType == StatementType.OTHER_DDL) {
            ...
            try {
                resultSets = Collections.singletonList(MetaResultSet.create(h.connectionId, h.id, false, statementHandle.getSignature(),
                        ...
                        maxRowsInFirstFrame != 0 && SEND_FIRST_FRAME_WITH_RESPONSE ? fetch(h, 0, (int) Math.min(Math.max(statementHandle.getMaxRowCount(), maxRowsInFirstFrame), Integer.MAX_VALUE)) : null //Frame.MORE // Send first frame to together with the response to save a fetch call
                ));
            } catch (NoSuchStatementException e) {
                String message = e.getLocalizedMessage();
                throw new AvaticaRuntimeException(message == null ? "null" : message, -1, "", AvaticaSeverity.ERROR);
    ...
    public List<AvaticaParameter> deriveAvaticaParameters(AlgDataType parameterRowType) {
        ...
        for (AlgDataTypeField field : parameterRowType.getFieldList()) {
            ...
            parameters.add(new AvaticaParameter(false, type.getPrecision() == AlgDataType.PRECISION_NOT_SPECIFIED ? 0 : type.getPrecision(), type.getScale() == AlgDataType.SCALE_NOT_SPECIFIED ? 0 : type.getScale(), type.getPolyType().getJdbcOrdinal(), type.getPolyType().getTypeName(), Object.class.getName(), field.getName()));
        }
        return parameters;
    }
    ...
    private class MonitoringPage {
        ...
        public MonitoringPage(String uniqueName) {
            ...
            connectionNumberTable = new InformationTable(informationGroupConnectionStatistics, Arrays.asList("Attribute", "Value"));
            ...
            connectionListTable = new InformationTable(informationGroupConnectionList, Arrays.asList("Connection ID", "User", "Driver", "TX", "Auto Commit", "Client IP"));
        ...
        public void updateConnectionListTable() {
            ...
            for (Entry<String, PolyphenyDbConnectionHandle> entry : openConnections.entrySet()) {
                ...
                connectionListTable.addRow(connectionId, connectionHandle.getUser().name, "N/A", txId, connectionHandle.isAutoCommit(), "N/A");
            }
        }
